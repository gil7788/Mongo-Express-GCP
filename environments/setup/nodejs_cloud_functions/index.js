...
//  // Retrieve the latest commit SHA from the remote repo without cloning
//  const mirroredRepoUrl = `https://source.developers.google.com/p/${projectId}/r/${mirroredRepoName}`;
//  const gitCommand = `git ls-remote ${mirroredRepoUrl} refs/heads/${branchName}`;

//  exec(gitCommand, (error, stdout, stderr) => {
//    if (error) {
//      console.error("Error retrieving commit SHA:", error);
//      res.status(500).send(`Error retrieving commit SHA: ${error.message}`);
//      return;
//    }
//    // Extract SHA from output, get first 7 characters for short SHA
//    const shaOutput = stdout.split('\t')[0]; 
//    const shortSHA = shaOutput.substring(0, 7);
...
//       steps: [
//         {
//           // Step 1: Clone the specific branch and store the cloudbuild.yaml
//           name: 'gcr.io/cloud-builders/git',
//           args: ['clone', '-b', branchName, mirroredRepoUrl, '.'],
//           id: 'clone-repo',
//         },
//         {
//           // Submitting a new build request with the short SHA included in the substitutions
...
//             '-c',
//             `
//             gcloud builds submit --config environments/cloudbuild.yaml --substitutions=_GCP_REGION=${substitutions._GCP_REGION},_PROJECT_ID=${substitutions._PROJECT_ID},_GCP_CONTAINER_REGISTRY_REPOSITORY_NAME=${substitutions._GCP_CONTAINER_REGISTRY_REPOSITORY_NAME},_DOCKER_IMAGE_NAME=${substitutions._DOCKER_IMAGE_NAME},_GCP_CLOUD_RUN_SERVICE_NAME=${substitutions._GCP_CLOUD_RUN_SERVICE_NAME},_SHORT_SHA=${shortSHA} ./app
//             `,
//           ],
//           id: 'submit-build',
//           waitFor: ['clone-repo'],
